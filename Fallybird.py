...
class Game:
    ...
    def obstacle_creation(self):
        ...
        if self.spawn_timer > self.spawn_interval:
            ...
            pipe_sprite3 = obstacle(1000, 805, self.gap_y, False)
            pipe_sprite4 = obstacle(1000, 135, abs(0.85 - self.gap_y), True)
    ...
    def remove_offscreen(self):
        for pipe in self.pipe:
            if pipe.rect.right <0 :
                ...
                if pipe in self.passed_pipe:
                    self.passed_pipe.remove(pipe)

    def difficulty_adjustment(self):
        level = (self.score/2) // 10
        self.spawn_interval = max(10, 75 - level * 10)
